# Execution Plan: Business Process Control Plane IPS adoption bootstrap
...
- Root IPS artifacts including new BUSINESS/SYSTEM/AGENT_OPERATIONS/CLAUDE docs
...
- No resolution of the four open project blockers described in AGENTS.md

## project invariants

This plan preserves BPCP-INV-001..005 by documenting them, not altering them.

## sensitive-data handling

No secrets are read, generated, or committed; BPCP_PROCESS_SIGNING_SECRET and BPCP_DATABASE_URL are referenced only by variable name.
...
- .env.example
- deploy.config.sh
- src/capabilities/capability-registry.service.ts
...
- Any application source code under src/ or process-registry/
- docs/orchestrator/* dated review/fix records
- k8s manifests
- deploy.config.sh
...
- Create the missing BUSINESS/SYSTEM/AGENT_OPERATIONS/CLAUDE docs and reformat every required artifact into validator-required sections using only observed facts
...
- None identified during this bootstrap; the four pre-existing open items in AGENTS.md remain project blockers, not bootstrap blockers.
