...
| Marketplace channel readback inventory | `business-process-control-plane` + channel repos | `docs/orchestrator/2026-07-06-marketplace-channel-business-health-inventory.md`; Aukro `GET /aukro/business-health/channel-readback`; Heureka `GET /heureka/business-health/channel-readback`; channel `src/business-health/**`; channel verifier scripts | partial source endpoints ready; remaining channel envelopes gated | static source endpoint only; no provider calls, no marketplace mutation, no production DB/API query |
