...
| Marketplace channel readback inventory | `business-process-control-plane` + channel repos | `docs/orchestrator/2026-07-06-marketplace-channel-business-health-inventory.md`; Aukro `GET /aukro/business-health/channel-readback`; Heureka `GET /heureka/business-health/channel-readback`; Allegro `GET /allegro/business-health/channel-readback`; Bazos `GET /bazos/business-health/channel-readback`; channel `src/business-health/**`; channel verifier scripts | partial source endpoints ready; FlipFlop envelope remains gated | static source endpoint only; no provider calls, no marketplace mutation, no production DB/API query |
...
| Suppliers traceability packet | `suppliers-microservice` | `GET /api/business-health/supplier-warehouse-traceability`; `src/business-health/**`; `scripts/verify-business-health-suppliers-traceability-contract.js`; `scripts/verify-business-health-suppliers-contract.js` | source endpoint ready; real supplier procurement facts gated | static source endpoint only; no production DB/API query, supplier import, Catalog write, or Warehouse mutation |
