...
- Consult STATE.json's `activeDocument` pointer (currently a dated docs/orchestrator/ validation report) before starting new work
- The four current blockers in AGENTS.md (downstream event consumers, Auth RBAC roles, editor ingress/domain, pricing/cart owner contract) must remain explicit `[MISSING: ...]` markers until an owner resolves them
